...
const CategoryMenu = (props: SiderProps) => {
  const { mini = false, onFilterCategory = () => {} } = props;
  return (
      ...
      {categoryList.map((item) => {
        return (
          <a
            ...
            className={`bg-white ${mini ? "p-2" : "p-6"} rounded-xl ${
              mini ? "" : "shadow-md"
            } text-center card-hover dark:bg-gray-700`}
          >
            <div
              className={`w-10 h-10 mx-auto ${
                mini ? "mb-1" : "mb-3"
              } rounded-full ${item.bgColor} flex items-center justify-center`}
            >
              {item.icon}
            </div>
            {mini ? null : (
              <h4 className="font-medium text-neutral">{item.categoryName}</h4>
            )}
